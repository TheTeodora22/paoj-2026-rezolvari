...
public class LibraryService {
            ...
            try (PreparedStatement checkPs = conn.prepareStatement(checkSql)) {
                ...
                try (ResultSet rs = checkPs.executeQuery()) {
                    ...
                    if (rs.getInt("available") == 0) {
                        throw new SQLException("Cartea nu e disponibila.");
                    }
                }
            }
    ...
    public List<String> getActiveLoansWithDetails() throws SQLException, IOException {
        ...
        try (PreparedStatement ps = getConn().prepareStatement(sql);
            ...
            while (rs.next()) {
                results.add(rs.getString("book_title") + " - " + rs.getString("reader_name")
                        + ", din " + rs.getString("loan_date"));
            }
        }
        return results;
    }
    ...
    public List<String> getTopBorrowedBooksWithAuthor() throws SQLException, IOException {
        ...
        try (PreparedStatement ps = getConn().prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                results.add(rs.getString("book_title") + " de " + rs.getString("author_name")
                        + ": " + rs.getLong("borrow_count") + " imprumuturi");
            }
        }
        return results;
    }
